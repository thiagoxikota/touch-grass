...
export function BeRealStamp({ timestamp, hours, src, alt, children, className }: BeRealStampProps) {
  return (
    <div className={cn('relative aspect-square bg-bg-alt border border-hairline overflow-hidden', className)}>
      {src ? (
        <img src={src} alt={alt ?? ''} className="absolute inset-0 w-full h-full object-cover" />
      ) : (
        children
      )}
      <div className="absolute top-4 left-4 px-3 py-2 bg-black border-2 border-earned font-mono text-[13px] font-black text-earned uppercase tracking-[0.12em]">
        // VERIFIED OFFLINE
      </div>
      ...
      <div className="absolute bottom-4 right-4 px-4 py-3 bg-earned text-black font-mono font-black leading-none">
